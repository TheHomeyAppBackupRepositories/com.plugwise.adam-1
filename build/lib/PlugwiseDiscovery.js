...
module.exports = class PlugwiseDiscovery extends SimpleClass {
  ...
  async onDiscoveryResult(discoveryResult) {
    ...
    discoveryResult.on('lastSeenChanged', discoveryResult => this.log('discoveryResult lastSeenChanged', discoveryResult));
    discoveryResult.on('addressChanged', discoveryResult => this.log('discoveryResult addressChanged', discoveryResult));
    ...
    await fetch(`http://${bridgeAddress}`).then(res => {
